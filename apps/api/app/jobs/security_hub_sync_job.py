"""
Security Hub Sync Job — ingests findings from AWS Security Hub across all enabled regions.

Lifecycle (Phase 3):
  1. Assume cross-account role via StsClient
  2. For each enabled region: iterate all active findings via SecurityHubClient
...
from app.integrations.aws.security_hub_client import SecurityHubClient
...
@register_job(JobType.SECURITY_HUB_SYNC)
class SecurityHubSyncJob(BaseJob):
    """
    Ingests Security Hub findings for a given AWS account across all enabled regions.
    ...
    async def run(self) -> JobResult:
        ...
        # Multi-region: prefer "regions" list, fall back to single "region"
        regions: list[str] = (
        ...
        self.emit_progress("Starting Security Hub sync: assuming role", pct=5)

        # ── Step 1: Assume Role ─────────────────────────────────────────────
        try:
            sts = StsClient.from_settings()
            session_name = f"CloudPostureSHSync-{str(uuid.uuid4())[:8]}"
        ...
        self.emit_progress(
            f"Role assumed; syncing Security Hub across {len(regions)} region(s)", pct=10
        )
        ...
        for idx, region in enumerate(regions):
            self.emit_progress(
                f"Security Hub: region {region} ({idx + 1}/{len(regions)})",
                pct=base_pct + idx * pct_per_region,
            )
            hub = SecurityHubClient(credentials=credentials, region=region)
            region_ingested = 0

            try:
                async for finding in hub.iter_active_findings():
                    try:
                        ...
                        if region_ingested % 50 == 0:
                            self.emit_progress(
                                f"Security Hub {region}: {region_ingested} findings",
                        ...
                        self._log.warning(
                            "security_hub_sync.finding_skip",
                ...
                self._log.error(
                    "security_hub_sync.region_error", region=region, error=str(exc)
                )
                progress["regions_processed"].append({
                    "region": region,
                    "ingested": 0,
                ...
                # Security Hub is a primary source — region failure is fatal
                progress["error"] = str(exc)
                return JobResult(
                    status=JobStatus.FAILED,
                    error=f"Security Hub ingestion failed in region {region}: {exc}",
                    output=progress,
                )
        ...
        try:
            ...
            self._log.error("security_hub_sync.normalization_error", error=str(exc))
            progress["normalization"] = {"error": str(exc)}

        self.emit_progress("Security Hub sync complete", pct=100)
